...
if(isset($_POST['add_room'])){
    ...
    if($stmt->execute()) $success = "Room added successfully!";
...
if(isset($_POST['edit_room'])){
    ...
    $stmt = $conn->prepare("UPDATE Room SET
        Room_HotelId=?, Room_Type=?, Room_Capacity=?,
        Room_PetFriendly=?, Room_Availability=? WHERE Room_Id=?");
    $stmt->bind_param("isiisi", $hotelId, $type, $capacity, $petFriendly, $availability, $id);

    if($stmt->execute()) $success = "Room updated successfully!";
    ...
    if(!in_array($file['type'], $allowed)){
        ...
        if(move_uploaded_file($file['tmp_name'], $destPath)){
            ...
            $stmt = $conn->prepare("INSERT INTO Room_Images
                (Image_RoomId, Image_Path, Image_IsCover, Image_Caption, Image_AddedDate)
                VALUES (?, ?, ?, ?, ?)");
            $stmt->bind_param("isiss", $roomId, $dbPath, $isCover, $caption, $addedDate);
            $stmt->execute();
...
$rooms          = $conn->query("SELECT r.*, h.Hotel_Name FROM Room r JOIN Hotel h ON h.Hotel_Id=r.Room_HotelId ORDER BY h.Hotel_Name, r.Room_Type");
$hotelsDropdown = $conn->query("SELECT Hotel_Id, Hotel_Name FROM Hotel ORDER BY Hotel_Name");
$roomsDropdown  = $conn->query("SELECT r.Room_Id, r.Room_Type, h.Hotel_Name FROM Room r JOIN Hotel h ON h.Hotel_Id=r.Room_HotelId ORDER BY h.Hotel_Name, r.Room_Type");
...
<style>
    ...
    .image-item .image-actions { position: absolute; bottom: 0; left: 0; right: 0; background: rgba(0,0,0,0.55); display: flex; justify-content: center; gap: 4px; padding: 4px; opacity: 0; transition: opacity 0.2s ease; }
    .image-item:hover .image-actions { opacity: 1; }
    .cover-badge { position: absolute; top: 4px; left: 4px; background: #1a8c55; color: #fff; font-size: 9px; font-weight: 700; padding: 2px 6px; border-radius: 4px; }
    .img-caption-label { position: absolute; bottom: 0; left: 0; right: 0; background: rgba(0,0,0,0.5); color: #fff; font-size: 9px; padding: 2px 4px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
</style>
...
<div class="admin-wrapper">
    ...
    <main class="admin-main">
        ...
        <div class="table-card mb-4">
            <div class="table-card-title"><span><i class="bi bi-plus-circle me-2" style="color:var(--trivago-blue);"></i>Add New Room</span></div>
            <form method="POST">
                <div class="row g-3">
                    <div class="col-md-4">
                    ...
                    <div class="col-12"><button type="submit" name="add_room" class="btn btn-trivago"><i class="bi bi-plus-circle me-1"></i>Add Room</button></div>
                </div>
            </form>
        </div>
        ...
        <!-- UPLOAD ROOM IMAGE -->
        <div class="table-card mb-4">
            <div class="table-card-title"><span><i class="bi bi-image me-2" style="color:#8e44ad;"></i>Upload Room Photo</span></div>
                ...
                <div class="row g-3 align-items-end">
                    <div class="col-md-3">
                        ...
                        <select name="image_room_id" class="form-control" required>
                            ...
                            <?php $roomsDropdown->data_seek(0); while($r=$roomsDropdown->fetch_assoc()): ?>
                                <option value="<?=$r['Room_Id']?>"><?=htmlspecialchars($r['Hotel_Name'])?> — <?=htmlspecialchars($r['Room_Type'])?> (ID: <?=$r['Room_Id']?>)</option>
                            <?php endwhile; ?>
                        </select>
                    </div>
                    <div class="col-md-3">
                        <label class="form-label fw-semibold">Choose Image (JPG, PNG, WEBP — max 5MB)</label>
                        <input type="file" name="room_image" class="form-control" accept="image/jpeg,image/png,image/webp" required>
                    </div>
                    <div class="col-md-3">
                        <label class="form-label fw-semibold">Caption <span class="text-muted">(e.g. Double Room, Bathroom)</span></label>
                        <input type="text" name="image_caption" class="form-control" placeholder="e.g. Double Room">
                    </div>
                    <div class="col-md-1 d-flex align-items-end pb-1">
                        <div class="form-check">
                            <input type="checkbox" name="is_cover" class="form-check-input" id="isCoverRoom">
                            <label class="form-check-label fw-semibold" for="isCoverRoom">Cover</label>
                        </div>
                    </div>
                    <div class="col-md-2">
        ...
        <div class="table-card">
            ...
            <div class="table-responsive">
                <table class="table admin-table mb-0">
                    <thead><tr><th>ID</th><th>Hotel</th><th>Type</th><th>Capacity</th><th>Pet Friendly</th><th>Availability</th><th>Photos</th><th>Actions</th></tr></thead>
                    <tbody>
                        <?php $rooms->data_seek(0); while($r=$rooms->fetch_assoc()):
                            $roomImages = $conn->query("SELECT * FROM Room_Images WHERE Image_RoomId={$r['Room_Id']} ORDER BY Image_IsCover DESC, Image_AddedDate ASC");
                        ?>
                        <tr>
                            <td><?=$r['Room_Id']?></td>
                            <td><?=htmlspecialchars($r['Hotel_Name'])?></td>
                            <td><span style="background:#e8f0fe;color:var(--trivago-blue);border-radius:6px;padding:3px 10px;font-size:12px;font-weight:600;"><?=htmlspecialchars($r['Room_Type'])?></span></td>
                            <td><i class="bi bi-person me-1" style="color:var(--trivago-blue);"></i><?=$r['Room_Capacity']?></td>
                            <td><?=$r['Room_PetFriendly']?'<i class="bi bi-check-circle-fill text-success"></i> Yes':'<i class="bi bi-x-circle-fill text-danger"></i> No'?></td>
                            <td><span class="availability-badge <?=$r['Room_Availability']==='Available'?'available':'unavailable'?>"><?=$r['Room_Availability']?></span></td>
                            <td>
                                ...
                                <div class="image-grid">
                                    ...
                                    <div class="image-item <?=$img['Image_IsCover']?'is-cover':''?>">
                                        ...
                                        <?php if(!empty($img['Image_Caption'])): ?><span class="img-caption-label"><?=htmlspecialchars($img['Image_Caption'])?></span><?php endif; ?>
                                        <div class="image-actions">
                                            <?php if(!$img['Image_IsCover']): ?>
                                            <a href="manage_rooms.php?set_cover=<?=$img['Image_Id']?>&room_id=<?=$r['Room_Id']?>" title="Set as cover" class="btn btn-sm btn-light p-0 px-1"><i class="bi bi-star-fill" style="font-size:11px;"></i></a>
                                            <?php endif; ?>
                                            <a href="manage_rooms.php?delete_image=<?=$img['Image_Id']?>" onclick="return confirm('Delete this image?')" class="btn btn-sm btn-danger p-0 px-1"><i class="bi bi-trash" style="font-size:11px;"></i></a>
                                        </div>
                                    </div>
                                    <?php endwhile; ?>
                                </div>
                            ...
                            <td>
                                <button class="btn btn-sm btn-outline-primary me-1" data-bs-toggle="modal" data-bs-target="#editRoomModal<?=$r['Room_Id']?>"><i class="bi bi-pencil"></i></button>
                                <a href="manage_rooms.php?delete=<?=$r['Room_Id']?>" class="btn btn-sm btn-outline-danger" onclick="return confirm('Delete this room?')"><i class="bi bi-trash"></i></a>
                            </td>
                        </tr>
                        ...
                        <div class="modal fade" id="editRoomModal<?=$r['Room_Id']?>" tabindex="-1">
                            <div class="modal-dialog modal-dialog-centered">
                                <div class="modal-content rounded-4">
                                    <div class="modal-header border-0"><h5 class="modal-title">Edit Room</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
                                        ...
                                        <form method="POST">
                                            ...
                                            <div class="row g-3">
                                                <div class="col-12">
                                                    ...
                                                    <select name="room_hotel_id" class="form-control" required>
                                                        <?php $hotelsDropdown->data_seek(0); while($h=$hotelsDropdown->fetch_assoc()): ?>
                                                            <option value="<?=$h['Hotel_Id']?>" <?=$h['Hotel_Id']==$r['Room_HotelId']?'selected':''?>><?=htmlspecialchars($h['Hotel_Name'])?></option>
                                                        <?php endwhile; ?>
                                                    </select>
                                                </div>
                                                <div class="col-md-6">
                                                    ...
                                                    <select name="room_type" class="form-control" required>
                                                        <?php foreach(['Single','Double','Twin','Suite','Deluxe','Family'] as $type): ?>
                                                            <option value="<?=$type?>" <?=$type==$r['Room_Type']?'selected':''?>><?=$type?></option>
                                                        <?php endforeach; ?>
                                                    </select>
                                                </div>
                                                <div class="col-md-6"><label class="form-label fw-semibold">Capacity</label><input type="number" name="room_capacity" class="form-control" min="1" max="20" value="<?=$r['Room_Capacity']?>" required></div>
                                                <div class="col-md-6">
                                                    <label class="form-label fw-semibold">Availability</label>
                                                    <select name="room_availability" class="form-control" required>
                                                ...
                                                <div class="col-md-6 d-flex align-items-end">
                                                    <div class="form-check mb-2">
                                                        <input type="checkbox" name="room_pet_friendly" class="form-check-input" id="petEdit<?=$r['Room_Id']?>" <?=$r['Room_PetFriendly']?'checked':''?>>
                                                        <label class="form-check-label fw-semibold" for="petEdit<?=$r['Room_Id']?>">Pet Friendly</label>
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="mt-3">
                                                <button type="submit" name="edit_room" class="btn btn-trivago"><i class="bi bi-check-circle me-1"></i>Save Changes</button>
